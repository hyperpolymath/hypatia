...
* *The unified-api-adapter wire contract cannot silently drift.* The Zig enum,
  the Idris2 ABI and the Rust client are pinned to one golden source
  (`ffi/connectors.json`) by `test/unified-api-adapter-contract_test.exs` under
  `mix test`.
...
* Deploy *verisim-api* (the VeriSimDB Rust core) to unlock native
  graph/vector/temporal modalities and let the unified-api-adapter transport bodies be
