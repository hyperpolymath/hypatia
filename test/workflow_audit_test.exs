...
defmodule Hypatia.Rules.WorkflowAuditTest do
  ...
  describe "lockfile-pinned actions" do
    @lock """
    ...
    test "the SAME ref without a lockfile is still reported unpinned" do
      # Guards the carve-out in the other direction: it must depend on the
      # lockfile existing, not silently accept every symbolic ref.
      [f] = WorkflowAudit.check_unpinned_actions(@wf)
      assert f.type == :unpinned_action
    end

    test "a ref ABSENT from the lockfile is still reported unpinned" do
      wf = %{"ci.yml" => "      - uses: some/other-action@v1\n"}
      [f] = WorkflowAudit.check_unpinned_actions(wf, actions_lock: @lock)
      assert f.type == :unpinned_action
      assert f.action_ref == "some/other-action@v1"
    end

    test "subpath actions match their root-repository lockfile entry" do
