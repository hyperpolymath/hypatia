...
\begin{table}[htbp]
\centering
\caption{Symbolic rule categories in \textsc{Hypatia}. Each rule is an Elixir predicate function with deterministic output, avoiding neural computation overhead for well-understood checks.}
\label{tab:rules}
\begin{tabular}{@{}llc@{}}
\toprule
...
\textbf{Nx/EXLA backend for scale.} The current neural layer runs in pure Elixir at reservoir sizes of 80--100, which is ample for decision-support at present volumes. If future workloads push reservoir sizes or retraining cadence beyond what pure Elixir can comfortably handle, migrating the tensor primitives to Nx with an EXLA backend is the natural next step: same BEAM runtime, same supervision tree, optional GPU acceleration, no new FFI boundary.
