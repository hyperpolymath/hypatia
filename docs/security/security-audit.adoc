...
= Security Audit Checklist for hypatia
:toc: macro
:toclevels: 3
:icons: font
:sectanchors:
...
This document provides a comprehensive security audit checklist for the hypatia neurosymbolic CI/CD intelligence platform. It covers all layers of the system: Rust adapters, Haskell verification registry, Logtalk rule engine, data stores (ArangoDB, Dragonfly), container deployments, and API surfaces.
...
Reference: `CRYPTO-STANDARD.scm` in the hypatia repo (`docs/CRYPTO-STANDARD.scm`)
...
Reference: `CII-ENFORCEMENT.adoc` in the hypatia repo
