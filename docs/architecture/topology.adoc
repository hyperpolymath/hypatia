...
[arabic]
. *Data seeding*: verisim-data dirs (patterns/recipes/outcomes) empty —
neural networks idle
. *Last-mile actuation*: gitbot-fleet detects problems but never commits
or creates PRs
. *HOOKSYNC_TOKEN*: .git-private-farm hookset propagation never tested
. *Recipe coverage*: Need to expand from initial seed recipes
