...
This guide covers deployment, operation, and maintenance of hypatia infrastructure for system administrators and DevOps engineers.

=== Architecture Summary

hypatia consists of several interconnected components:
...
# Create namespace
kubectl create namespace hypatia

# Create secrets
kubectl create secret generic hypatia-secrets \
  --namespace hypatia \
...
# Install the chart
helm install hypatia hyperpolymath/hypatia \
  --namespace hypatia \
...
ingress:
  ...
  hosts:
    - host: registry.hypatia.example.com
      paths:
        - path: /
          service: registry
  tls:
...
[source,bash]
----
kubectl get all -n hypatia
----

Expected output:
----
NAME                                    READY   STATUS    RESTARTS   AGE
pod/hypatia-adapter-xxx            1/1     Running   0          1h
pod/hypatia-engine-xxx             1/1     Running   0          1h
pod/hypatia-registry-xxx           1/1     Running   0          1h
pod/arangodb-coordinator-xxx            1/1     Running   0          1h
pod/dragonfly-xxx                       1/1     Running   0          1h

NAME                              TYPE        CLUSTER-IP       PORT(S)
service/hypatia-registry     ClusterIP   10.96.xxx.xxx    8080/TCP
service/hypatia-engine       ClusterIP   10.96.xxx.xxx    8081/TCP
service/hypatia-adapter      ClusterIP   10.96.xxx.xxx    8082/TCP
...
----
# Copy binaries
cp registry/dist-newstyle/build/.../hypatia /usr/local/bin/
cp adapters/target/release/hypatia-adapter /usr/local/bin/
cp cli/target/release/hyper /usr/local/bin/

# Create systemd services
cp deploy/systemd/*.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable hypatia-registry hypatia-engine hypatia-adapter
----
...
----
# Kubernetes
kubectl scale deployment hypatia-registry --replicas=5
...
----
# Add coordinator
helm upgrade hypatia hyperpolymath/hypatia \
...
[INPUT]
    Name              tail
    Path              /var/log/containers/*hypatia*.log
    Parser            docker
    Tag               hypatia.*
...
tls:
  enabled: true
  secretName: hypatia-internal-tls
  clientAuth: required
----
...
metadata:
  ...
  annotations:
    ...
    iam.gke.io/gcp-service-account: hypatia@project.iam.gserviceaccount.com
----
...
vault:
  ...
  role: hypatia
  secrets:
    - path: secret/data/hypatia/github
      key: token
      env: GITHUB_TOKEN
----
...
audit:
  ...
  maxBackups: 10
  maxSize: 100
  path: /var/log/hypatia/audit.log
----
...
----
# Run migrations
kubectl exec -it deployment/hypatia-registry -- \
  hypatia migrate --up
----
...
[source,bash]
----
kubectl logs -f deployment/hypatia-registry --tail=100
----
...
[source,bash]
----
kubectl exec -it deployment/hypatia-engine -- \
