...
The hypatia platform exposes a REST API for programmatic access to the ruleset registry, verification services, and fleet operations. This document supplements the existing `api-reference.adoc` with additional endpoint details.

== Base URL

Production::
  `https://api.hypatia.io/v1`
...
curl -H "Authorization: Bearer $TOKEN" \
     https://api.hypatia.io/v1/rulesets
...
curl -X POST \
     ...
     https://api.hypatia.io/v1/fleet/runs
----
...
async fn list_rulesets(token: &str) -> Result<Vec<Ruleset>, Error> {
    ...
    let response = client
        .get("https://api.hypatia.io/v1/rulesets")
        .bearer_auth(token)
        .send()
        .await?;
...
async fn start_fleet_run(token: &str, repo: &str) -> Result<FleetRun, Error> {
    let client = Client::new();
    let response = client
        .post("https://api.hypatia.io/v1/fleet/runs")
        .bearer_auth(token)
        .json(&json!({
            "repository": repo,
