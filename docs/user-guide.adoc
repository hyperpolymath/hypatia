...
*hypatia* is a neurosymbolic CI/CD intelligence platform that combines neural learning with symbolic reasoning to provide automated repository management, security enforcement, and policy compliance.
...
Before installing hypatia, ensure you have:
...
[source,bash]
----
curl -fsSL https://releases.hyperpolymath.dev/hypatia/latest/hyper-darwin-arm64 -o hyper
chmod +x hyper
sudo mv hyper /usr/local/bin/
----
...
[source,bash]
----
curl -fsSL https://releases.hyperpolymath.dev/hypatia/latest/hyper-darwin-x86_64 -o hyper
chmod +x hyper
sudo mv hyper /usr/local/bin/
----
...
This creates `~/.config/hypatia/config.toml` with default settings.
...
The configuration file is located at `~/.config/hypatia/config.toml`.

[source,toml]
----
# Registry settings
[registry]
url = "https://registry.hypatia.hyperpolymath.dev"
cache_ttl = 3600
...
Create `.hypatia.toml` in your repository root to override settings:
...
hypatia includes several pre-built rulesets:
...
blockTypeScript = PreventiveRule
  ...
  (RejectCommit "TypeScript not allowed. Use AffineScript instead.")
----
...
[source,bash]
----
curl -I https://registry.hypatia.hyperpolymath.dev/health
----
...
2. Search existing issues: https://github.com/hyperpolymath/hypatia/issues
