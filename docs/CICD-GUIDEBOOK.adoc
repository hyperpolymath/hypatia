...
[abstract]
This guidebook provides comprehensive documentation for the hypatia platform and its integration with gitbot-fleet. It covers the philosophy, architecture, tooling, error patterns, and operational procedures for maintaining CI/CD compliance across the hyperpolymath repository ecosystem.
...
The hypatia platform is a neurosymbolic CI/CD intelligence system that:
...
cargo build --release --package hypatia-fixer
...
The gitbot-fleet bots can leverage hypatia for automated fixes:
...
| *DevOps Engineer*
| hypatia maintenance, bot configuration
...
Applied by cicd-fixer from hypatia."
...
vim $REPOS_DIR/hypatia/.audittraining/SHA-PINS.scm
# Add:
#     (owner/action
#      "abc123def456..."
#      "v2.0.0")
...
vim $REPOS_DIR/hypatia/fixer/src/sha_pins.rs
# Add:
...
vim $REPOS_DIR/hypatia/.audittraining/ERROR-CATALOG.scm
# Add:
#     (new-error-id
...
vim $REPOS_DIR/hypatia/fixer/src/catalog.rs
# Add Rust equivalent

# Step 4: Update scanner.rs (if auto-detectable)
vim $REPOS_DIR/hypatia/fixer/src/scanner.rs
# Add detection logic

# Step 5: Update fixer.rs (if auto-fixable)
vim $REPOS_DIR/hypatia/fixer/src/fixer.rs
# Add fix logic
...
[qanda]
What is hypatia?::
hypatia is a neurosymbolic CI/CD intelligence platform that learns from CI/CD failures, distills patterns into rules, and applies fixes programmatically across repositories.
...
Dependabot handles dependency updates but doesn't address workflow security (SHA pinning, permissions), SPDX headers, or cross-repo template propagation. hypatia complements Dependabot.
...
Report it at https://github.com/hyperpolymath/hypatia/issues. In the meantime, revert the affected workflows and fix manually.
...
* link:../README.adoc[hypatia README]
...
_This guidebook is maintained as part of hypatia and should be updated when patterns, SHA pins, or procedures change._
