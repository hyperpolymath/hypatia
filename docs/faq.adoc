...
Frequently asked questions about hypatia, organized by topic.
...
hypatia uses neural learning to identify patterns in CI/CD failures across thousands of repositories, then converts these patterns into symbolic rules that execute in microseconds without ML inference overhead.

=== How is hypatia different from GitHub Actions / GitLab CI?

hypatia is not a CI/CD execution platform -- it's an intelligence layer that works *with* your existing CI/CD:

[cols="2,3,3"]
|===
| Aspect | CI/CD Platforms | hypatia
...
=== What forges does hypatia support?
...
=== Is hypatia open source?

Yes. hypatia is dual-licensed under:

* *PMPL-1.0-or-later* (legal)
* *Palimpsest Licence* (philosophical)

The source code is available at https://github.com/hyperpolymath/hypatia
...
[source,bash]
----
curl -fsSL https://releases.hyperpolymath.dev/hypatia/latest/hyper-linux-x86_64 -o hyper
chmod +x hyper
...
No. You can use the public registry at `registry.hypatia.hyperpolymath.dev` without deploying any infrastructure.
...
Yes. Create `.hypatia.toml` in your repository:
...
=== Is hypatia safe to run on my repositories?
...
The neural → symbolic distillation ensures rules execute without ML inference overhead.

=== Can hypatia handle large organizations?

Yes. hypatia is designed for scale:
...
=== Can I use hypatia in CI/CD pipelines?
...
=== Does hypatia work with monorepos?
...
Or scan the entire monorepo with per-directory rules in `.hypatia.toml`.

=== Can I use hypatia with pre-commit hooks?
...
* *GitHub issues*: https://github.com/hyperpolymath/hypatia/issues
