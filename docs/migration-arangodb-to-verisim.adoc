...
=== verisim-data (Git-Backed, Canonical)

The `verisim-data` repository is a Git-backed flat-file store that serves as
the canonical source of truth for security intelligence data. Hypatia reads from
it via the VCL FileExecutor.
...
[cols="1,1,1"]
|===
| Capability | verisim-data | ArangoDB
...
The `verisim-api` project (Rust, in `verisim/rust-core`) implements a
...
| Structured document storage with VCL-native querying; replaces
...
| **VCL layer**
| Already integrated, query syntax does not change. The VCL Client will gain
  an HTTP executor (see migration step 2) but existing FileExecutor and all
  `Hypatia.VCL.Query` functions remain compatible.
...
* Build from `verisim/rust-core`
* Configure graph, vector, temporal, and document modalities
* Seed with existing data from `verisim-data` flat files
* Validate that the API responds to basic VCL-over-HTTP queries

=== Step 2: Add HTTP Executor to VCL Client

Extend `lib/vcl/client.ex` to support an HTTP executor alongside the existing
FileExecutor.

[source,elixir]
----
# The VCL Client will route queries to the appropriate executor:
#   - FileExecutor: local verisim-data reads (existing)
#   - HttpExecutor: verisim-api REST/gRPC calls (new)
#   - ArangoDB: graph queries (transitional, to be removed)
----

The HTTP executor sends parsed VCL ASTs to the `verisim-api` endpoint and
...
* Update VCL Client to remove ArangoDB executor routing
...
| verisim-data Git repo retains all canonical data; can re-seed
...
| VCL compatibility
| Subtle differences in how verisim-api interprets VCL vs FileExecutor.
...
modality work is needed in `verisim/rust-core`, the timeline extends
accordingly.
