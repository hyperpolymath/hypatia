...
*Status:* ✅ IMPLEMENTED *Date:* 2026-02-06 *Version:* 1.0.0

=== Overview

Hypatia now integrates with: - *a2ml* (AI Assistant Markup Language) -
AI governance and workflow documentation - *K9 Contractiles* (Nickel
configuration) - Fleet-wide deployment and configuration management

This integration provides: 1. *Centralized Configuration:* Single source
of truth for fleet settings 2. *AI Assistant Integration:* Standardized
workflow for AI-driven rule management 3. *Type-Safe Config:* Nickel
validates configuration at parse time 4. *Deployment Automation:* K9
contractiles enable automated fleet deployment
...
|`+HYPATIA_RULE_PROPOSAL_THRESHOLD+` |`+5+` |Observations for rule
proposal
...
|`+HYPATIA_ECHIDNA_URL+` |`+http://localhost:8080+` |ECHIDNA validation
endpoint
...
The `+rules.a2ml+` file provides structured instructions for AI
assistants:

*When adding new patterns:* 1. Add detection logic to `+hypatia-cli.sh+`
2. Deploy to fleet 3. Monitor observations 4. Generate Logtalk rule when
threshold crossed (5+ obs) 5. Validate with ECHIDNA (90%+ score
required) 6. Deploy to `+approved-rules/+`

*When modifying rules:* 1. Update both Logtalk rule and bash pattern 2.
Re-validate with ECHIDNA 3. Document changes in deployment manifest
...
*Usage:* - Hypatia submits findings to
`+{shared_context_path}/findings/+` - Fleet coordinator aggregates
observations - Learning loop reads from
`+{shared_context_path}/learning/+`
...
*Usage:* - Submit rule proposals to ECHIDNA for semantic validation -
Require 90%+ validation score for approval - ECHIDNA checks logical
consistency, coverage, interactions
...
*Usage:* - Auto-fix scripts placed in `+auto_fix_dir+` - Automaton
executes fixes when approved - `+supervised+` mode requires human
approval per fix
...
*4. When threshold crossed (5 obs), generate rule:* - Create Logtalk
rule in `+rule-proposals/+` - Validate with ECHIDNA - Deploy to
`+approved-rules/+`
...
The `+rules.a2ml+` file is human-readable documentation. Validate by: 1.
Ensuring workflow instructions are current 2. Checking file paths are
correct 3. Verifying metrics match `+rule-deployment-status.json+`
...
*Problem:* `+read-k9-config.sh+` fails silently *Solution:* Check if
Nickel is installed:
...
*Problem:* Pattern enabled in K9 but not detecting *Solution:* Check
pattern is also implemented in `+hypatia-cli.sh+`
...
*Problem:* ECHIDNA validation < 90% *Solution:* Check validation report
...
*Last Updated:* 2026-02-06T22:45:00+00:00 *Maintained By:* Hypatia
