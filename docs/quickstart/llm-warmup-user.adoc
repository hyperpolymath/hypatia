...
hyperpolymath ecosystem. It scans repositories for weak points, routes
findings through a safety triangle (Eliminate > Substitute > Control),
and dispatches fixes to a fleet of bots with Bayesian confidence
scoring.
...
Prerequisites: Elixir (>= 1.14), Erlang/OTP (>= 25), Rust (nightly),
just.
...
* *Batch rollback*: Rollback entire dispatch batches with confidence
revert
...
* *Depends on*: panic-attacker (scanning), verisim-data (data), proven
(verified safety)
* *Coordinates*: gitbot-fleet (rhodibot, echidnabot, sustainabot,
glambot, seambot, finishbot)
* *Downstream*: All hyperpolymath repos benefit from automated scanning
+ fixing
