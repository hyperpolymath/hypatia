...
Core OTP application with 8 GenServers supervised by
`+Hypatia.Application+`.

[width="100%",cols="48%,52%",options="header",]
|===
|Module |Purpose
|`+pattern_analyzer.ex+` |Full pipeline orchestrator: scan -> patterns
-> triangle -> dispatch

|`+verisimdb_connector.ex+` |VCL-powered data access with file I/O
fallback

|`+pattern_registry.ex+` |Deduplicates findings into canonical patterns
(PA001-PA020)

|`+recipe_matcher.ex+` |Fuzzy matching: fingerprinted IDs to clean
recipe IDs

|`+triangle_router.ex+` |Routes through Eliminate > Substitute > Control
hierarchy

|`+fleet_dispatcher.ex+` |Confidence-gated dispatch (file-based + HTTP,
circuit breaker)

|`+dispatch_manifest.ex+` |Writes JSONL manifests as bridge to bash
execution

|`+outcome_tracker.ex+` |Records fix outcomes, Bayesian confidence
updating, re-scan verification

|`+recipe_generator.ex+` |Auto-generates recipes for uncovered
categories at 0.50 confidence

|`+scorecard_ingestor.ex+` |Ingests 20 OpenSSF Scorecard checks as local
scan patterns

|`+learning_scheduler.ex+` |GenServer: polls outcomes every 5 min,
drives feedback loop
...
|`+query.ex+` |High-level: fetch_scans, cross_repo_patterns,
pipeline_health
|===
...
|`+quarantine.ex+` |3 levels: soft/hard/permanent. Triggers on 5+
failures or >30% FP

|`+batch_rollback.ex+` |Rollback entire dispatch batches with confidence
revert
|===
...
|`+GRPC.idr+` |gRPC service definitions (scanner, dispatch, stream,
health)
...
verisim-data is a separate git-backed flat-file repo. Hypatia reads it
via `+VerisimdbConnector+` using VCL queries.
...
[arabic]
. *Eliminate* — Can the root cause be removed entirely?
. *Substitute* — Can a safer alternative replace the current approach?
. *Control* — Apply defensive measures (least preferred)

`+TriangleRouter.route()+` implements this hierarchy. Higher-level
strategies get higher confidence scores.
...
Confidence tiers: - >= 0.95: auto_execute via robot-repo-automaton -
0.85-0.94: review via rhodibot (creates PR) - < 0.85: report_only via
sustainabot (advisory)

Bayesian Beta-distribution confidence: prior_strength=10, floor=0.10,
cap=0.99.
...
From `+mix.exs+`: - `+jason+` — JSON parsing - `+gen_stage+` —
Back-pressure for pipeline stages - `+phoenix+` — Web framework (for
future HTTP dashboard) - `+bandit+` — HTTP server - `+plug+` — HTTP
middleware

=== Known Gaps (for context)

[arabic]
. VCL federation local-only (FileExecutor, not multi-store)
. verisim-api not deployed (graph/vector/temporal via flat files only)
. One-sided training data (99%+ success — needs failure data)
. 310/600 auto-execute entries have null fix_script
. Neural state persistence not yet writing to disk
. Ada TUI not wired into OTP supervision
...
[arabic]
. Add to `+PatternRegistry+` (unique PA code, e.g., PA021)
. Add recipe in `+recipes/+` with fix script
. Add triangle routing logic
. Run `+mix test+` to verify
. Outcomes will update confidence via Bayesian feedback

=== Machine-Readable Metadata

All in `+.machine_readable/6a2/+`: - `+STATE.a2ml+`, `+META.a2ml+`,
`+ECOSYSTEM.a2ml+` - `+AGENTIC.a2ml+`, `+NEUROSYM.a2ml+`,
`+PLAYBOOK.a2ml+`
