...
learns to skip comment lines (token vs syntactic match).
|===

=== Audit-training and remediation-script corpora

The following directories contain example secrets / vulnerability
patterns _by design_ (training corpora for the scanner itself, and
...
* `+test/**+` and `+**/tests/**+` — test fixtures that contain deliberately
  invalid credential-shaped examples.
