...
implementations working against verisim-data flat files. Health check,
...
outcomes are "success") — balanced learning improves as failure data accumulates.
...
. **[RESOLVED] VCL: Test suite complete.** +
223 tests across 3 files: `vcl_client_test.exs` (74), `vcl_file_executor_test.exs`
(58), `vcl_query_test.exs` (91). Full suite: 298 tests, 0 failures.
...
. **[PARTIAL] VCL federation: Git-clone-based multi-store implemented.** +
The FileExecutor now supports `{:remote, url}` sources via `RemoteCache`, which
clones or pulls remote verisim-data repos into a TTL-managed local cache
(`/tmp/hypatia-vcl-cache/`). VCL syntax: `FROM FEDERATION REMOTE "url"` or
`FROM REMOTE "url" STORE scans`. Thread-safe via ETS locking. +
*Remaining:* Federation across verisim-api HTTP instances and heterogeneous
backends (non-git stores) is not yet implemented.
...
temporal, and vector modalities. See `docs/migration-arangodb-to-verisim.adoc`
...
dispatches, or interact with the VCL layer. Integration requires an IPC
...
The `verisim-api` service (built from `verisim/rust-core`) is not deployed
...
See `docs/migration-arangodb-to-verisim.adoc` for deployment plans.
...
broken. Manual ingestion via helper scripts (`verisim-data/scripts/ingest-scan.sh`)
