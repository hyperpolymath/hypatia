...
. **[PARTIAL] VQL federation: Git-clone-based multi-store implemented.** +
The FileExecutor now supports `{:remote, url}` sources via `RemoteCache`, which
clones or pulls remote verisimdb-data repos into a TTL-managed local cache
(`/tmp/hypatia-vql-cache/`). VQL syntax: `FROM FEDERATION REMOTE "url"` or
