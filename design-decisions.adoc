...
|Registry and API in a sanctioned language (Ephapax/Rust/Elixir — ReScript
 ...
 (see `hyperpolymath/snifs`) when crossing the BEAM boundary. Hypatia's
 own FFI is Zig-via-dlopen, not NIFs — SNIFs become relevant only if a
 BEAM NIF is ever added (e.g. alongside a future Nx/EXLA backend).
...
- link:../verisim/WHITEPAPER.md[VeriSimDB White Paper]
- link:../verisim/docs/deployment-modes.adoc[Deployment Modes]
- link:../verisim/docs/challenges-federated.adoc[Federated Challenges]
- link:../verisim/docs/technical-specification-kraft-metadata-log.adoc[KRaft Metadata Log]
