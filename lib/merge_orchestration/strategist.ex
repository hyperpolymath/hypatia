  ...
  defp symbolic_vetoes(ctx) do
    ...
    vetoes =
      if Map.get(ctx, :license_touch, false) do
        [%{bot: "policy-gate", reason: "license/SPDX -- owner-only"} | vetoes]
      else
        vetoes
      end
    ...
    vetoes =
      if String.contains?(title_lower, "do not merge") || String.contains?(title_lower, "wip") do
        [%{bot: "policy-gate", reason: "title contains DO NOT MERGE or WIP"} | vetoes]
      else
        vetoes
      end
    ...
    vetoes =
      if Enum.any?(labels_lower, &(&1 in hold_labels)) do
        [%{bot: "policy-gate", reason: "PR has do-not-merge or hold label"} | vetoes]
      else
        vetoes
      end
    ...
    vetoes =
      if String.starts_with?(branch, "litmus/") || String.starts_with?(branch, "test/") do
        [%{bot: "policy-gate", reason: "branch prefix litmus/ or test/"} | vetoes]
      else
        vetoes
      end
    ...
    vetoes =
      if state == "draft" do
        [%{bot: "policy-gate", reason: "PR is in draft state"} | vetoes]
      else
        vetoes
      end
