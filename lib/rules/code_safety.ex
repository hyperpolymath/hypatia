...
defmodule Hypatia.Rules.CodeSafety do
  ...
  @rescript_patterns [
    ...
    %{
      id: :json_decode_no_validation,
      severity: :critical,
      pattern: ~r/JSON\.parseExn/,
      cwe: "CWE-20",
  ...
  @affine_hand_port_patterns [
    %{
      id: :handle_as_fn_name,
      severity: :high,
      pattern: ~r/(?:^|\n)\s*(?:pub\s+)?(?:total\s+)?fn\s+handle\s*[\(\<]/,
      cwe: "CWE-1109",
      description:
        "Function named `handle` collides with AffineScript HANDLE keyword token (used by `handle body { handlers }` effect-handler expression form). Rename to `dispatch`, `handle_request`, `handle_event`, etc."
    },
    %{
      id: :ocaml_style_float_div,
      severity: :high,
      pattern: ~r/[a-zA-Z0-9_)\]]\s*\/\.\s/,
      cwe: "CWE-704",
      description:
        "OCaml-style float division `/.` is not accepted by AffineScript — use unified `/` for both Int and Float"
    },
    %{
      id: :ocaml_style_float_mul,
      severity: :high,
      pattern: ~r/[a-zA-Z0-9_)\]]\s*\*\.\s/,
      cwe: "CWE-704",
      description:
        "OCaml-style float multiplication `*.` is not accepted by AffineScript — use unified `*` for both Int and Float"
    },
    %{
      id: :ocaml_style_float_add,
      severity: :high,
      pattern: ~r/[a-zA-Z0-9_)\]]\s*\+\.\s/,
      cwe: "CWE-704",
      description:
        "OCaml-style float addition `+.` is not accepted by AffineScript — use unified `+` for both Int and Float"
    },
    %{
      id: :ocaml_style_float_sub,
      severity: :high,
      pattern: ~r/[a-zA-Z0-9_)\]]\s*-\.\s/,
      cwe: "CWE-704",
      description:
        "OCaml-style float subtraction `-.` is not accepted by AffineScript — use unified `-` for both Int and Float"
    }
  ]
  ...
  @ada_spark_patterns [
    %{
      id: :ada_pragma_suppress,
      severity: :high,
      pattern: ~r/pragma\s+Suppress/,
      cwe: "CWE-704",
      description: "Ada pragma Suppress disables runtime checks"
    },
    %{
      id: :ada_unchecked_conversion,
      severity: :high,
      pattern: ~r/Unchecked_Conversion/,
      cwe: "CWE-704",
      description: "Ada Unchecked_Conversion bypasses type safety"
    },
    %{
      id: :ada_unchecked_deallocation,
      severity: :high,
      pattern: ~r/Unchecked_Deallocation/,
      cwe: "CWE-401",
      description: "Ada Unchecked_Deallocation manual memory management"
    },
    %{
      id: :ada_unchecked_access,
      severity: :high,
      pattern: ~r/Unchecked_Access/,
      cwe: "CWE-676",
  ...
  @elixir_patterns [
    ...
    %{
      id: :elixir_system_cmd_dynamic_binary,
      severity: :high,
      pattern: ~r/System\.cmd\(\s*"[^"]*#\{/,
      cwe: "CWE-78",
    ...
    %{
      id: :elixir_system_shell,
      severity: :critical,
      pattern: ~r/System\.shell\([^)]*#\{/,
      cwe: "CWE-78",
    ...
    %{
      id: :elixir_code_eval,
      severity: :critical,
      pattern: ~r/Code\.eval_string|Code\.eval_quoted|Code\.eval_file/,
      cwe: "CWE-94",
      description: "Code.eval_* -- arbitrary code execution risk"
    },
    %{
      id: :elixir_send_unsanitised,
      severity: :high,
      pattern: ~r/:erlang\.binary_to_term\(/,
      cwe: "CWE-502",
      description: "binary_to_term without :safe option -- deserialization attack"
    },
    %{
      id: :elixir_atom_from_user,
      severity: :high,
      pattern: ~r/String\.to_atom\(/,
      cwe: "CWE-400",
      description:
        "String.to_existing_atom with user input exhausts atom table -- use to_existing_atom"
    },
    %{
      id: :elixir_wildcard_plug_cors,
      severity: :high,
      pattern: ~r/origin:\s*"\*"/,
      cwe: "CWE-942",
    ...
    %{
      id: :elixir_no_ssl_verify,
      severity: :high,
      pattern: ~r/verify:\s*:verify_none|ssl:\s*\[verify:\s*:verify_none\]/,
      cwe: "CWE-295",
      description: "SSL verify_none disables certificate validation -- MITM risk"
    },
    %{
      id: :elixir_port_open_shell,
      severity: :high,
      pattern: ~r/Port\.open\(\{:spawn,/,
      cwe: "CWE-78",
      description: "Port.open spawn -- validate command before execution"
    },
    %{
      id: :elixir_send_resp_no_escape,
      severity: :medium,
      pattern: ~r/send_resp\(.*#\{/,
      cwe: "CWE-79",
  ...
  @erlang_patterns [
    %{
      id: :erlang_binary_to_term_unsafe,
      severity: :critical,
      pattern: ~r/\b(?:erlang:)?binary_to_term\(/,
      cwe: "CWE-502",
      description: "binary_to_term without safe decoding options -- deserialization attack"
    },
    %{
      id: :erlang_atom_from_untrusted,
      severity: :high,
      pattern: ~r/\b(?:erlang:)?(?:binary_to_atom|list_to_atom)\(/,
      cwe: "CWE-400",
      description: "Atom creation from untrusted input may exhaust atom table"
    },
    %{
      id: :erlang_command_exec,
      severity: :high,
      pattern: ~r/\bos:cmd\(|open_port\(\s*\{spawn,|erlang:open_port\(\s*\{spawn,/,
      cwe: "CWE-78",
      description: "Shell command execution from Erlang runtime -- validate input and avoid shell"
    },
    %{
      id: :erlang_tls_verify_none,
      severity: :high,
      pattern: ~r/ssl:connect\([^)]*\{verify,\s*verify_none\}/s,
      cwe: "CWE-295",
      description: "TLS verification disabled in ssl:connect -- MITM risk"
    },
    %{
      id: :erlang_insecure_httpc,
      severity: :medium,
      pattern: ~r/httpc:request\(\s*\"http:\/\//,
      cwe: "CWE-319",
  ...
  @javascript_patterns [
    ...
    %{
      ...
      severity: :critical,
      pattern: ~r/(api_key|apiKey|secret|password|token)\s*[:=]\s*["'][a-zA-Z0-9+\/=]{8,}["']/,
      cwe: "CWE-798",
    ...
    %{
      ...
      cwe: "CWE-338",
      description:
        "Math.random() used for security-sensitive identifier/token generation -- use crypto.randomUUID or getRandomValues"
    }
  ]

  @shell_patterns [
    %{
      id: :shell_download_then_run,
      severity: :high,
      pattern: ~r/\b(?:curl|wget)\b[^\n|;]*\|\s*(?:sh|bash)\b/,
      cwe: "CWE-494",
      description:
        "Download-and-execute pattern (curl|wget pipe to shell) -- verify integrity before execution"
    },
    %{
      id: :shell_process_substitution_download_exec,
      severity: :high,
      pattern: ~r/\b(?:sh|bash)\s+<\(\s*(?:curl|wget)\b/,
      cwe: "CWE-494",
      description:
        "Process-substitution download-and-execute detected -- avoid executing remote scripts directly"
    }
  ]
  ...
  @stub_crypto_patterns [
    %{
      id: :stub_crypto_function,
      severity: :critical,
      pattern: ~r/stub:(sha|blake|argon|dilithium|kyber|chacha|hkdf|shake)/,
      cwe: "CWE-327",
    ...
    %{
      id: :todo_crypto,
      severity: :high,
      pattern: ~r/TODO.*(?:Replace with real|implement|stub).*(?:hash|crypt|sign|verify|kdf)/i,
      cwe: "CWE-327",
    ...
    %{
      id: :fake_signature,
      severity: :critical,
      pattern: ~r/fake|placeholder|dummy.*(?:signature|key|hash|cert|token)/i,
      cwe: "CWE-327",
  ...
  @nickel_patterns [
    ...
    %{
      id: :ncl_weak_hash,
      severity: :high,
      pattern: ~r/(md5:|sha1:)[a-fA-F0-9]+/,
      cwe: "CWE-328",
      description: "Weak hash (MD5/SHA-1) in config -- use SHA-256+"
    },
    %{
      id: :ncl_action_version_tag,
      severity: :medium,
    ...
    %{
      id: :ncl_banned_language_ref,
      severity: :high,
      pattern: ~r/language\s*=\s*"(typescript|go|python|java|kotlin|swift|dart)"/,
      cwe: "CWE-1104",
    ...
    %{
      id: :ncl_k9_missing_pedigree,
      severity: :high,
      pattern: ~r/\AK9!(?!.*pedigree\s*=).+\z/s,
      cwe: "CWE-1104",
      description: "K9 contractile missing pedigree section"
    },
    %{
      id: :ncl_hardcoded_secret,
      severity: :critical,
    ...
    %{
      id: :ncl_docker_not_podman,
      severity: :low,
      pattern: ~r/docker\s|docker\.io|dockerfile/i,
      cwe: "CWE-1104",
      description:
        "Docker reference in Nickel config -- Podman/Containerfile highly preferred (Docker permitted)"
    }
  ]
  ...
  @container_patterns [
    %{
      id: :env_var_injection,
      severity: :critical,
      pattern: ~r/Command::new\(.*env::var/s,
      cwe: "CWE-78",
    ...
    %{
      ...
      severity: :high,
      pattern: ~r/user_input.*[\/\\]/,
      cwe: "CWE-22",
      description: "Path traversal risk with user input"
    },
    %{
      id: :permissive_bypass,
      severity: :high,
      pattern: ~r/[Pp]ermissive.*continue|[Aa]udit.*mode/,
      cwe: "CWE-863",
  ...
  @banned_file_extensions [
    %{
      ...
      glob: "*.py",
      language: "Python",
      replacement: "Julia/Rust/AffineScript",
  ...
  def check_scm_locations(file_list) do
    Enum.flat_map(@scm_file_names, fn scm ->
      ...
      |> Enum.map(fn f ->
        %{
          rule: :scm_wrong_location,
          severity: :critical,
          description: "#{scm} found outside #{@scm_canonical_dir}/",
          file: f,
          expected_dir: @scm_canonical_dir
        }
      end)
    end)
  end
  ...
  def check_dockerfile_naming(file_list) do
    ...
    |> Enum.map(fn f ->
      %{
        rule: :dockerfile_not_containerfile,
        severity: :low,
        description:
          "Dockerfile detected -- Containerfile highly preferred (Docker permitted; rename unless a specific tool requires the Dockerfile name)",
        file: f
      }
    end)
  end
  ...
  def scan_container_code(content) do
    Enum.flat_map(@container_patterns, fn rule ->
      if Regex.match?(rule.pattern, content) do
        [%{rule: rule.id, severity: rule.severity, cwe: rule.cwe, description: rule.description}]
      else
        []
      end
    end)
