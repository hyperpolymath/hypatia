...
defmodule Hypatia.Rules.DependabotAlerts do
    ...
    case fetch_alerts(owner, repo) do
      {:ok, alerts} ->
        ...
        |> Enum.map(fn alert ->
          ...
          ecosystem =
            get_in(alert, ["security_vulnerability", "package", "ecosystem"]) || "unknown"
          ...
          if critical_count > 0 do
            [
              %{
                ...
                severity: :critical,
                reason:
                  "#{critical_count} critical Dependabot alert(s) -- immediate patching required",
                action: :escalate,
          ...
          if high_count >= 5 do
            [
              %{
                ...
                severity: :high,
                reason:
                  "#{high_count} high-severity Dependabot alert(s) -- batch update recommended",
                action: :batch_update,
          ...
          if total >= 10 do
            [
              %{
                ...
                severity: :medium,
                reason:
                  "#{total} total open Dependabot alert(s) -- dependency hygiene review needed",
                action: :review,
                detail: %{
                  total: total,
                  by_severity: Map.new(by_severity, fn {k, v} -> {k, length(v)} end)
                }
              }
              | findings
            ]
      ...
      {:ok, alerts} ->
        ...
        |> Enum.map(fn alert ->
          ...
          %{
            ...
            severity: :high,
            reason:
              "Dependabot alert for #{package} (#{severity}) is #{age} days old (threshold: #{threshold} days) -- overdue for remediation",
            action: :escalate,
            detail: %{
              alert_number: alert["number"],
              package: package,
      ...
      {:ok, alerts} ->
        ...
        |> Enum.map(fn alert ->
          ...
          %{
            ...
            severity: :medium,
            reason:
              "Dependabot alert for #{package} (#{severity}) dismissed as '#{reason}' -- ensure risk is documented",
            action: :review,
            detail: %{
              alert_number: alert["number"],
              package: package,
  ...
  def classify_alert(alert) when is_map(alert) do
    ...
    cond do
      state == "dismissed" and dismissed_reason in ["tolerable_risk", "no_bandwidth", nil] ->
        {:control,
         "dismissed as '#{dismissed_reason || "no reason"}' -- human acceptance of risk"}
  ...
  defp classify_bump(from_range, to_version)
       when is_binary(from_range) and is_binary(to_version) do
  ...
  defp fetch_alerts(owner, repo) do
    ...
    if token == nil or token == "" do
      ...
      url =
        "#{@github_api_base}/repos/#{owner}/#{repo}/dependabot/alerts?state=open&per_page=#{@max_alerts_per_repo}"
  ...
  defp extract_owner_repo(repo_path) do
    case System.cmd("git", ["remote", "get-url", "origin"], cd: repo_path, stderr_to_stdout: true) do
      {url, 0} ->
