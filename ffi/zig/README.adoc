...
* `src/unified-api-adapter.zig` — 16-protocol UnifiedApiAdapter surface + dispatch seam (see below)
...
== UnifiedApiAdapter Surface

Beyond the seven core C ABI functions, `src/unified-api-adapter.zig` defines the
*UnifiedApiAdapter*: a sixteen-protocol unified API surface that wraps the
...
renumbered. Port layout is `base + id + 1` (`UnifiedApiAdapter.portFor`).
...
`test/unified-api-adapter-contract_test.exs` (run under `mix test`) fails CI if any of
