...
For the WASM bindings: - Sandbox file system access - Limit memory
allocation - No network access from config evaluation

'''''

=== Fuzzing Targets

bunsenite should fuzz: 1. Config parsing (malformed Nickel) 2. FFI
boundaries (invalid inputs from other languages) 3. WASM bindings
(memory safety)
