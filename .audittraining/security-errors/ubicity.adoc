...
Workflow lacks explicit `+permissions:+` declaration, inheriting default
(often write) permissions.
...
|`+.github/workflows/rescript-deno-ci.yml+` |Actions using `+@v4+` not
SHA

|`+.github/workflows/rsr-antipattern.yml+` |Actions using `+@v4+` not
SHA
|===
