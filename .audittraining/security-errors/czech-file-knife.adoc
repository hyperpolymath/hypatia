...
. Update Cargo.toml with patched versions
. Run `+cargo update+` to update lockfile
. Verify with `+cargo audit+` again
...
czech-file-knife is a Rust workspace with 8 crates: - cfk-core -
cfk-providers - cfk-cache - cfk-search - cfk-vfs - cfk-cli -
cfk-integrations - cfk-ios
