    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::from_default_env().add_directive(
                "cicd_fixer=info"
                    .parse()
                    .expect("invariant: 'cicd_fixer=info' is a well-formed tracing directive"),
            ),
        )
        .init();
            ...
            if format == "json" {
                ...
                println!(
                    ...
                    serde_json::to_string_pretty(&json)
                        .expect("invariant: serde_json::Value always serialises")
                );
            } else {
...
fn batch_process(fixer: &CicdFixer, repos_dir: &PathBuf, fix: bool, dry_run: bool, limit: usize) {
        ...
        .take(limit)
    {
        ...
        processed += 1;
        // Falls back to the full path when the directory iterator yielded an
        // entry without a file name (e.g. root "/"); shouldn't happen for
        // repo paths but we don't want to panic the whole sweep on a fluke.
