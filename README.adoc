...
Philosophy: **Palimpsest**. The Palimpsest-MPL (PMPL) text is provided in `LICENSES/MPL-2.0.txt`, and the canonical source is the palimpsest-license repository.
...
* *Graph Intelligence*: verisim-data git-backed flat-file store (ArangoDB ditched per #273 architecture audit) for relationship modelling; queried via the in-process VCL parser + FileExecutor.
...
[source,bash]
----
helm repo add hypatia https://hyperpolymath.github.io/hypatia
helm install hypatia hypatia/hypatia
----
...
hypatia deposit ./my-ruleset.hs \
  --name "rust-security-hardening" \
  --description "Security rules for Rust projects" \
  --sign --verify

# Withdraw for local use
hypatia withdraw rust-security-hardening \
  --version "1.2.0" \
  --output ./rules/

# Search for rulesets
hypatia search --effect preventive --language rust

# Audit a ruleset
hypatia audit rust-security-hardening --full
----
...
cicd_hyper_a:
  enabled: true
  registry_url: https://registry.hypatia.hyperpolymath.dev
...
----
#!/bin/bash
# .git/hooks/pre-commit (injected by hypatia)

# Load rulesets from registry (cached in Dragonfly)
hypatia hooks pre-commit \
...
This project is licensed under link:https://github.com/hyperpolymath/palimpsest-license[MPL-2.0] (Palimpsest-MPL License).
...
See link:docs/architecture/topology.md[docs/architecture/topology.md] for a visual architecture map and completion dashboard.
